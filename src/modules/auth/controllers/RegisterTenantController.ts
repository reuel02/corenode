...
export async function registerTenantController(request: FastifyRequest, reply: FastifyReply) {
    const registerBodySchema = z.object({
        companyName: z.string().min(2, "O nome da empresa precisa ter no minimo 2 caracteres"),
        userName: z.string().min(2, "O nome do usuário precisa ter no minimo 2 caracteres."),
    ...
    try {
        ...
        const registerService = new RegisterTenantService(usersRepository, tenantsRepository)
